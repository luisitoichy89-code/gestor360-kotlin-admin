...
@Composable
fun AdminUsersScreen(    clienteId: String,
    onBack: () -> Unit,
        ...
        Column(modifier = Modifier.fillMaxSize().padding(paddingValues)) {
            ...
            if (uiState.isLoading) {
                ...
                LazyColumn(verticalArrangement = Arrangement.spacedBy(8.dp), contentPadding = PaddingValues(horizontal = 16.dp, vertical = 8.dp)) {
                    items(filteredUsers, key = { it.id }) { user ->                        Card(modifier = Modifier.fillMaxWidth()) {
    ...
    if (showDialog) {
        ...
        var newAlmacenId by remember { mutableStateOf("") }; var newDeviceId by remember { mutableStateOf("") }        AlertDialog(onDismissRequest = { showDialog = false }, title = { Text("👤 Nuevo usuario") },
            text = { Column {
